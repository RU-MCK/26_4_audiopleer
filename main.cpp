...
class Player {
 ...
 public:
  ...
  void Play(std::string &name) {
    ...
    if (!play) {
      for (int i = 0; (i < track.nameTrack.size()); ++i) {
        if (track.nameTrack[i] == name) {
          std::cout << "Track name: " << track.nameTrack[i] << ". "
                    << "Track time: " << track.timeTrack[i] << " sec. "
                    << "Track date: " << track.dateTrack[i] << " year." << std::endl;
          play = true;
          break;
        }
      }
  ...
  void Next() {
    if (play) {
      int ind;
      srand(time(nullptr));
      ind = rand() % track.nameTrack.size();
      std::cout << "Track name: " << track.nameTrack[ind] << ". "
                << "Track time: " << track.timeTrack[ind] << " sec. "
                << "Track date: " << track.dateTrack[ind] << " year." << std::endl;
      pause = false;
    }
  }
};
